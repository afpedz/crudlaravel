            ...
            <form id="loginForm" class="space-y-3" action="{{ route('login') }}" method="post">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email"
                        class="w-full px-4 py-2 mt-1 border rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 @error('email') border-red-500 @enderror"
                ...
                <div>
                    ...
                    <input type="password" name="password" id="password"
                        class="w-full px-4 py-2 mt-1 border rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 @error('password') border-red-500 @enderror"
                ...
                <button type="submit"
                    class="w-full px-4 py-2 text-white bg-gray-800 rounded-lg hover:bg-gray-900">{{ $title }}</button>
            </form>
            <p class="text-sm text-center text-gray-600">Don't have an account? <a href="/register"
                    class="text-gray-600 hover:underline font-semibold">Sign up</a></p>
        </div>
    </div>
    <script>
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 422) {
                            ...
                            $.each(errors, function(key, value) {
                                ...
                                input.after('<div class="text-red-500 mt-2 text-sm">' +
                                    value[0] + '</div>'); // Show error message
                            });
                        } else if (xhr.status === 401) {
                            // Handle invalid login credentials
                            const statusMessage = xhr.responseJSON.message ||
