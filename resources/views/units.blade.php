<x-layout>
        ...
        <table class="w-full border-collapse border border-gray-300 shadow-lg rounded-lg overflow-hidden">
            <thead>
                <tr class="bg-gray-800 text-white">
                    <th class="border border-gray-300 px-5 py-2 w-1/6">ID</th>
                    <th class="border border-gray-300 px-5 py-2 w-2/3">Name</th>
                    <th class="border border-gray-300 px-5 py-2 w-1/6">Action</th>
                </tr>
            </thead>
                ...
                @if ($units->count() > 0)
                    @foreach ($units->sortBy('id') as $unit)
                        <tr class="border border-gray-300 text-gray-700 hover:bg-gray-50 item"
                            data-id="{{ $unit->id }}">
                            <td class="border border-gray-300 px-5 py-2 text-center w-1/6">{{ $unit->id }}</td>
                            <td class="border border-gray-300 px-5 py-2 w-2/3">{{ $unit->name }}</td>
                            <td class="border border-gray-300 px-5 py-2 text-center w-1/6">
                                <!-- Edit Button -->
    ...
    <script>
        ...
        document.getElementById('addUnitForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    addUnitToTable(data.unit);
                    closeUnitModal();
                } else {
                    console.error('Error adding unit:', data);
                }
            })
            .catch(error => console.error('Error:', error));
        });

        // AJAX for editing a unit
        document.getElementById('editUnitForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    updateUnitInTable(data.unit);
                    closeEditModal();
                } else {
                    console.error('Error updating unit:', data);
                }
            })
            .catch(error => console.error('Error:', error));
        });

        // AJAX for deleting a unit
        document.getElementById('deleteUnitForm').addEventListener('submit', function(e) {
            e.preventDefault();
            const formData = new FormData(this);
            fetch(this.action, {
                method: 'POST',
            ...
            .then(data => {
                if (data.success) {
                    removeUnitFromTable(data.id);
                    closeDeleteModal();
                } else {
                    console.error('Error deleting unit:', data);
                }
            })
            .catch(error => console.error('Error:', error));
        });

        // Helper functions to manipulate the table
        function addUnitToTable(unit) {
            const unitTableBody = document.getElementById('unitTableBody');
            const newRow = document.createElement('tr');
            newRow.className = 'border border-gray-300 text-gray-700 hover:bg-gray-50 item';
            newRow.setAttribute('data-id', unit.id);
            newRow.innerHTML = `
                <td class="border border-gray-300 px-5 py-2 text-center w-1/6">${unit.id}</td>
                <td class="border border-gray-300 px-5 py-2 w-2/3">${unit.name}</td>
                <td class="border border-gray-300 px-5 py-2 text-center w-1/6">
                    <div class="flex justify-center gap-3">
                        <button onclick="showEditModal('${unit.id}', '${unit.name}')" class="text-gray-800 hover:underline">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                            </svg>
                        </button>
                        <span>|</span>
                        <button onclick="showDeleteModal('${unit.id}')" class="text-red-500 hover:underline">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        function updateUnitInTable(unit) {
            const unitRow = document.querySelector(`tr[data-id='${unit.id}']`);
            if (unitRow) {
                unitRow.children[1].textContent = unit.name;
            }
        }

        function removeUnitFromTable(id) {
